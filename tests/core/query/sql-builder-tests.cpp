...
int main()
{
  ...
  const worm::core::Column schemaEmail{
    worm::reflection::FieldMetadata{.columnName = "email", .defaultExpression = "'unknown'", .nullable = false},
    schemaUsers};
  ...
  if (pgCreate.size() != 2 ||
      pgCreate[0].sql != "create table \"public\".\"users\" (\"id\" bigint generated by default as identity not "
                         "null,\"email\" varchar(120) default 'unknown' not null,constraint \"pk_users\" primary key "
                         "(\"id\"))" ||
  ...
  if (mySqlCreate[0].sql.find("`id` bigint auto_increment not null") == std::string::npos ||
      mySqlCreate[0].sql.find("`email` varchar(120) default 'unknown' not null") == std::string::npos ||
      sqliteCreate[0].sql.find("\"id\" integer primary key autoincrement") == std::string::npos ||
      sqliteCreate[0].sql.find("\"email\" text default 'unknown' not null") == std::string::npos ||
      ...
      sqlServerCreate[0].sql.find("[email] nvarchar(120) default 'unknown' not null") == std::string::npos) {
  ...
  try {
    const worm::core::Column generatedDefaultColumn{
      worm::reflection::FieldMetadata{.columnName = "generated_default", .defaultExpression = "0", .generated = true},
      schemaUsers};
    const worm::core::TableMetadata generatedDefaultMetadata{schemaUsers,
      {worm::core::ColumnMetadata{generatedDefaultColumn, {.kind = worm::core::ColumnTypeKind::Int64}}}};
    static_cast<void>(pgBuilder.create(generatedDefaultMetadata));
    std::cerr << "CREATE TABLE accepted a generated column with a default expression.\n";
    return 1;
  } catch (const worm::SqlBuildException&) {}

  try {
    const worm::core::Column unsafeColumn{
      worm::reflection::FieldMetadata{.columnName = "unsafe", .defaultExpression = "0; drop table users"},
      schemaUsers};
    const worm::core::TableMetadata unsafeMetadata{schemaUsers,
      {worm::core::ColumnMetadata{unsafeColumn, {.kind = worm::core::ColumnTypeKind::Int32}}}};
    static_cast<void>(pgBuilder.create(unsafeMetadata));
    std::cerr << "CREATE TABLE accepted an unsafe default expression.\n";
    return 1;
  } catch (const worm::SqlBuildException&) {}
