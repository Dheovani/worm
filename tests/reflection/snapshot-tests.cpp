...
int main()
{
  ...
  if (worm::reflection::is_dirty(entity, snapshot) || worm::reflection::changed_field_count(entity, snapshot) != 0) {
  ...
  if (changed != 2 || changedNames != std::vector<std::string>{"id", "name"} || !idValuesAreCorrect ||
      !nameValuesAreCorrect || worm::reflection::changed_field_count(entity, snapshot) != 2 ||
