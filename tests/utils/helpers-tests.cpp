...
int main()
{
    ...
    static_assert(std::is_same_v<
        utils::remove_class_pointer_t<decltype(&Derived::value), Derived>,
        int
    >);
    ...
    try {
        ...
        if (variables.at("database_type") != "sqlite"
            || variables.at("host") != "localhost"
            || utils::env::FindEnvInProjectRoot() != (root / ".env").string()
        ...
        try {
            utils::env::LoadFromPath((root / "missing.env").string());
