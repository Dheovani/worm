...
namespace worm::reflection
{
        ...
        [&](const auto&... descriptor) {
          (
            [&] {
              ...
              if (!result && std::invoke(hasher, candidate) == requestedHash && candidate == name) {
                result = index;
              }
              ++index;
            }(),
      ...
      std::apply(
        [&](const auto&... descriptor) {
          (
            [&] {
              const std::string_view candidate = std::invoke(selectName, descriptor);
              if (!visited && std::invoke(hasher, candidate) == requestedHash && candidate == name) {
  ...
  template <Reflectable T, typename Visitor, typename Hasher = FieldNameHasher>
  constexpr bool visit_field(T&& object, std::string_view name, Visitor&& visitor, Hasher hasher = {})
  {
    return visit_field_descriptor<std::remove_cvref_t<T>>(
      name,
      [&object, &visitor](const auto& descriptor) { std::invoke(visitor, descriptor, descriptor.get(object)); },
      hasher);
  }
} // namespace worm::reflection
