...
namespace worm
{
  namespace reflection
  {
    namespace detail
    {
      ...
      template <typename Entity, std::size_t... Indexes>
      [[nodiscard]] constexpr auto snapshot_type(std::index_sequence<Indexes...>) -> std::tuple<
        std::remove_cv_t<typename std::tuple_element_t<
          Indexes,
          std::remove_cvref_t<decltype(Entity::reflect())>>::value_type>...>;
    ...
    constexpr std::size_t for_each_changed_field(const T& object, const EntitySnapshot<T>& snapshot, Visitor&& visitor)
    {
      using Entity = std::remove_cvref_t<T>;
