...
namespace worm::reflection
{
  ...
  template <Reflectable T, typename Visitor> constexpr void for_each_field(T&& object, Visitor&& visitor)
  {
    const auto fields = std::remove_cvref_t<T>::reflect();

    std::apply([&object, &visitor](const auto&... field) { (std::invoke(visitor, field, field.get(object)), ...); },
               fields);
  }
} // namespace worm::reflection
